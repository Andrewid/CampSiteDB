...
--query2		   
DECLARE @RangeTill date = '2018-8-25'
DECLARE @RangeFrom date = '2018-7-10'
SELECT 
	cg.CampgroundID, cs.CampSiteID, 
	CONCAT(SUM(DATEDIFF(day,CheckIn_Date,CheckOut_Date)) * 100 / DATEDIFF(day,@RangeFrom,@RangeTill),'%') AS 'Utility'
FROM 
	CampSites cs INNER JOIN CampGrounds cg
		ON cs.CampgroundID = cg.CampgroundID INNER JOIN Reservations r
